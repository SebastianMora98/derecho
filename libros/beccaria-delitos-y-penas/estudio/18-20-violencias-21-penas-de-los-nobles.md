...
```mermaid
flowchart TD
    V["Delitos contra la persona: pena corporal, nunca precio"]:::critico
    C["Si se paga, el hombre deja de ser persona y se reputa cosa"]
    T["El poderoso busca las combinaciones que la ley pone a su favor"]
    O["Tiranía oculta: el insecto que carcome las compuertas"]
    N["La pena del noble es la del último ciudadano"]:::critico
    P["Toda distinción legítima supone una igualdad previa ante la ley"]
    S["Objeción: no sufre lo mismo"]
    D["Respuesta: la medida es el daño público, no la sensibilidad"]
    V --> C
    C --> T
    T --> O
    P --> N
