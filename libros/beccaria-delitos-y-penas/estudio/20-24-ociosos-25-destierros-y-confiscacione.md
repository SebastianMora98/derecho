...
```mermaid
flowchart TD
    T["Quien no obedece las leyes debe ser excluido"]
    OP["Ocio político: no aporta ni trabajo ni riqueza"]:::critico
    OU["Ocio de las riquezas bien adquiridas: útil, no punible"]
    L["Debe definirlo la ley, no los censores"]
    D["Destierro por gran probabilidad, con derecho a probar la inocencia"]
    C["¿Confiscar los bienes del desterrado?"]
    N["Aunque produzca un bien, la pena debe ser necesaria"]:::critico
    R["Pone precio a los flacos y castiga a la familia inocente"]
    T --> OP
    T --> OU
    OP --> L
    T --> D
