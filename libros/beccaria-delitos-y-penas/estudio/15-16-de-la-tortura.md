...
```mermaid
flowchart TD
    N["Nadie es reo antes de la sentencia"]
    D["Dilema: si el delito es cierto la confesión sobra; si es incierto se tortura a un inocente"]:::critico
    M["El dolor mide la resistencia, no la verdad"]
    A["Absuelve al malvado robusto y condena al inocente flaco"]
    O["Igual que las ordalías: la voluntad cede a la impresión sensible"]
    I["El inocente queda en peor condición que el reo"]:::critico
    P["Purgar la infamia con dolor: se la quita causándola"]
    C["Para los cómplices tampoco sirve: huyen solos"]
    N --> D
    D --> M
    M --> A
    M --> O
