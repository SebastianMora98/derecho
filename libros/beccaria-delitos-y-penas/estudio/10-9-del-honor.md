...
```mermaid
flowchart TD
    L["Las leyes nacen contra el despotismo físico de cada hombre"]
    F["Aparece una franja que la ley no alcanza"]:::critico
    O["Ahí nace el despotismo de la opinión"]
    N["La consideración ajena se vuelve necesaria"]
    H["El honor nació después del pacto: no está en el depósito común"]
    V["Reclamarlo es una vuelta momentánea al estado natural"]
    E["Por eso desaparece en la libertad extrema y en la dependencia extrema"]
    L --> F
    F --> O
    O --> N
    N --> H
