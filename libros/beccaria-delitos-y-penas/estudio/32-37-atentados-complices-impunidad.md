...
```mermaid
flowchart TD
    A["Atentado: acción que manifiesta la voluntad"]
    M["Pena menor que la consumación"]:::critico
    R["Así queda lugar al arrepentimiento"]
    C["Cómplices no ejecutores: también pena menor"]
    E["El ejecutor corre más riesgo: más difícil encontrarlo"]
    U["Objetivo: dejar los menos motivos de acuerdo posibles"]
    I["Impunidad al delator: balance de ventajas e inconvenientes"]
    G["Mejor una ley general que una promesa caso por caso"]
    D["Y aun así, remordimiento: la ley autoriza la traición"]:::critico
    A --> M
    M --> R
    M --> C
    C --> E
