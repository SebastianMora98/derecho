...
```mermaid
flowchart TD
    H["La gente recibe las leyes por tradición, no por examen"]
    O["Las leyes debían ser pactos y fueron urgencias de pocos"]:::critico
    C["Criterio: la mayor felicidad en el mayor número"]
    P["Lo penal quedó sin examinar en casi toda Europa"]
    M["Sigue a Montesquieu pero reclama sus propios pasos"]
    L["Legitima examinar el sistema penal"]
    H -->|"explica que el abuso durara siglos"| O
    O -->|"exige una vara de medida"| C
    O --> P
    C --> P
    P --> M
    M --> L
