...
```mermaid
flowchart TD
    S["Acusación secreta admitida por flaqueza de la constitución"]
    F["Hace a los hombres falsos y dobles"]:::critico
    E["Cada uno ve en el otro un posible delator"]
    C["Sin soldados intrépidos ni magistrados incorruptos"]
    J["Justificaciones: salud pública, indemnidad, infamia, naturaleza del delito"]
    R["Todas se refutan: el secreto es el escudo de la tiranía"]
    P["Remedio: acusación pública y comisarios"]
    T["Al calumniador, la pena que tocaba al acusado"]
    S --> F
    F --> E
    E --> C
    S --> J
