...
```mermaid
flowchart TD
    P["Delitos de prueba difícil: menos probables"]
    X["La práctica los prueba con semipruebas y tortura"]
    C["Debería ser al revés: examen y prescripción más cortos"]:::critico
    O["Y hay que mirar el origen de cada conducta"]
    A["Adulterio: necesidad permanente, mal repartida"]
    V["Venus ática: efecto de una educación que encierra"]
    I["Infanticidio: elección entre la infamia y la miseria"]
    R["La pena es incentivo si el delito suele quedar impune"]
    F["No es justa la pena si la ley no intentó prevenir"]:::critico
    P --> X
    X --> C
    C --> O
    O --> A
